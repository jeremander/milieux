...
class TestDistro:
    ...
    def test_lock(self, monkeypatch, tmp_config):
        ...
        # use local package
        output = [f'Distro {name} already exists -- overwriting', f'Wrote {name} requirements']
        check_main(['distro', 'new', name, '--packages', 'file://project1', '-f'], stderr=output)
...
class TestEnv:
    ...
    def test_install(self, monkeypatch, tmp_config):
        ...
        reqs_path.write_text('file://project2')
